...
fn occluder(p: vec2<i32>, sd: ptr<function, SD>) -> vec3<f32> {
    ...
    let r: f32 = pixel_radius;
    *sd = map(uv);
    let d: f32 = (*sd).d / r;
    if (d > 1.0) {
        return vec3<f32>(0.0);
    }
    let n: vec2<f32> = normal_map(uv, r);
...
fn propagate(p: vec2<i32>, n: vec2<f32>, sa: f32, sd: SD, colorChannel: i32, img_input: texture_2d<f32>) -> vec3<f32> {
    ...
    var occ_sd: SD;
    let occ: vec3<f32> = occluder(p, &occ_sd);

    let F: vec3<f32> = textureLoad(img_input, p, 0).rgb;
    let dV = vec3<f32>(n, 1) * SH1_Basis;
    ...
    let dRV = vec3<f32>(-n,1) * SH1_Basis;
    var ref2 = 0.0;
    if (sd.emissive) {
...
fn lpv_kernel(fragCoord: vec2<i32>, colorChannel: i32, img_input: texture_2d<f32>) -> vec4<f32> {
    var fragColor = vec3<f32>(0.);
    ...
    fragColor +=
     propagate(p+vec2<i32>(-d, 0), vec2<f32>(1., 0.),  sa1, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>(-d, 0), dn1,                sa2, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>(-d, 0), dn0,                sa2, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>( d, 0), vec2<f32>(-1., 0.), sa1, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>( d, 0), -dn0,               sa2, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>( d, 0), -dn1,               sa2, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>(0, -d), vec2<f32>(0., 1.),  sa1, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>(0, -d), dn1.yx,             sa2, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>(0, -d), dn0.yx,             sa2, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>(0,  d), vec2<f32>(0., -1.), sa1, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>(0,  d), -dn1.yx,            sa2, sd, colorChannel, img_input)
    +propagate(p+vec2<i32>(0,  d), -dn0.yx,            sa2, sd, colorChannel, img_input);
    return vec4<f32>(fragColor, 0.0);
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    fragColor = lpv_kernel(vec2<i32>(global_id.xy), 0, img_inputR);
    textureStore(img_outputR, vec2<i32>(global_id.xy), fragColor);

    fragColor = lpv_kernel(vec2<i32>(global_id.xy), 1, img_inputG);
    textureStore(img_outputG, vec2<i32>(global_id.xy), fragColor);

    fragColor = lpv_kernel(vec2<i32>(global_id.xy), 2, img_inputB);
    textureStore(img_outputB, vec2<i32>(global_id.xy), fragColor);
}
