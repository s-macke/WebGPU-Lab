...
@group(0) @binding(0) var scene: texture_2d_array<f32>;
@group(0) @binding(1) var<storage, read_write> state: array<Cell>; // this is used as both input and output
...
fn main(data: VertexOutput) -> @location(0) vec4f {
    var iResolution = vec2f(512, 512);
    let p = vec2i(data.fragUV*iResolution);

    var idx = u32(p.x + p.y * 512);
