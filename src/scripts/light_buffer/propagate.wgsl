...
struct Cell {
    // speherical harmonics for n=0 and n=1
    chr: vec3f,
    chg: vec3f,
    chb: vec3f,

    Em : vec3f, // emitted light
    translucency: f32
};

@group(0) @binding(0) var<uniform> staging: StagingBuffer;
@group(1) @binding(0) var scene : texture_2d_array<f32>;
@group(2) @binding(0) var<storage, read_write> state: array<Cell>; // this is used as both input and output
...
fn retrieveConstants(pn: vec2i, F: ptr<function, ColorCH>,  E: ptr<function, ColorCH>) {
    var idx = u32(pn.x + pn.y * 512);

    *F = ColorCH(
        state[idx].chr, // incoming red light from neighbor as circular harmonics
        state[idx].chg, // incoming green light from neighbor as circular harmonics
        state[idx].chb, // incoming blue light from neighbor as circular harmonics
    );
    /*
    *E = ColorCH(
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var idx = u32(global_id.x + global_id.y * 512);
    state[idx].chr = outch.r;
    state[idx].chg = outch.g;
    state[idx].chb = outch.b;
}
